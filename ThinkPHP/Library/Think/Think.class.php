...
class Think {
    ...
    static public function appException($e) {
        ...
        Log::record($e->getMessage(), Log::ALERT);
        Log::record(var_export(IS_GET ? $_GET : $_POST, true), Log::ALERT);
        Log::record($error['trace'], Log::ALERT);
        //线上环境加入slack通知
        if (!APP_DEBUG) {
            Log::save('slack', 'lemon-exception');
        }
        // 发送404信息
    ...
    static public function halt($error) {
        ...
        if (isset($_ENV['ENVIROMENT']) && $_ENV['ENVIROMENT'] == 'production') {
            echo json_encode(['error_code' => -100, 'error_msg' => '系统遇到了一些问题', 'data' => '']);
            exit();
        }
        // 包含异常页面模板
